...
const TicTacToe = ({ squares = arr }) => {
  ...
  useEffect(() => {
    ...
    const declareWinner = winner => {
      ...
      switch (winner) {
        case PLAYER_X:
          winnerStr = "Pemain 1 menang!";
          break;
        case PLAYER_O:
          winnerStr = "Pemain 0 menang!";
          break;
        case DRAW:
        default:
          winnerStr = "Tak ada pemenang!";
      }
  ...
  return gameState === GAME_STATES.notStarted ? (
    <Screen>
      <Inner>
        <ChooseText>Tingkat Kesulitan</ChooseText>
      ...
      <Inner>
        <ChooseText>Pilih jagoanmu!</ChooseText>
        <ButtonRow>
          <button onClick={() => choosePlayer(PLAYER_X)}>1</button>
          <p>|</p>
    ...
    <Container dims={DIMS}>
      {grid.map((value, index) => {
        ...
        return (
          ...
          >
            {isActive && <Marker>{value === PLAYER_X ? "1" : "0"}</Marker>}
          </Square>
        );
      })}
      <Strikethrough
...
const Container = styled.div`
  ...
  backround-color : rgb(127, 218, 218);
`;
